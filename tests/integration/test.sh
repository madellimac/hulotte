...
# Dependencies used by the legacy integration matrix.
STREAMPU_ROOT="$REPO_ROOT/vendor/streampu"
AFF3CT_ROOT="$REPO_ROOT/vendor/aff3ct"

# Test output directory. By default, generated projects stay outside Hulotte;
# set HULOTTE_TEST_OUTPUT_DIR to retain them for debugging.
if [[ -n "${HULOTTE_TEST_OUTPUT_DIR:-}" ]]; then
    OUTPUT_DIR="$(cd "$HULOTTE_TEST_OUTPUT_DIR" 2>/dev/null && pwd || printf '%s' "$HULOTTE_TEST_OUTPUT_DIR")"
    rm -rf "$OUTPUT_DIR"
    mkdir -p "$OUTPUT_DIR"
    CLEAN_OUTPUT=0
else
    OUTPUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hulotte-tests.XXXXXX")"
    CLEAN_OUTPUT=1
fi
...
elif [ -f "$(dirname "$REPO_ROOT")/.venv/bin/python" ]; then
    PYTHON_CMD="$(dirname "$REPO_ROOT")/.venv/bin/python"
fi
...
for aff3ct in "no-aff3ct" "aff3ct"; do
    for custom in "no-custom" "custom"; do
        for hw in "no-hw" "hw"; do
            ...
            $PYTHON_CMD "$FRAMEWORK_DIR/create_project.py" \
                --name "$PROJ_NAME" \
...
run_test() {
    ...
    if ! "$PYTHON_CMD" "$FRAMEWORK_DIR/create_project.py" --validate-manifest ./hulotte.project.json > manifest_log.txt 2>&1; then
...
run_add_scripts_idempotence_checks() {
    ...
    if ! $PYTHON_CMD "$FRAMEWORK_DIR/create_project.py" \
        --name "$id_proj" \
        --output-dir "$OUTPUT_DIR" \
        --no-aff3ct \
        --no-custom \
    ...
    if ! "$FRAMEWORK_DIR/add_hardware_module.py" --project-root "$proj_dir" --name IdemBlock --id idem_hw_main > "$proj_dir/idem_hw_run1.log" 2>&1; then
        echo "  [FAIL IDEM] add_hardware first run failed (see $proj_dir/idem_hw_run1.log)"
        FAILED=1
        return
    fi
    "$FRAMEWORK_DIR/add_hardware_module.py" --project-root "$proj_dir" --name IdemBlock --id idem_hw_main > "$proj_dir/idem_hw_run2.log" 2>&1
    local rc_hw=$?
    if [ "$rc_hw" -ne 2 ]; then
    ...
    if ! "$FRAMEWORK_DIR/add_custom_module.py" --project-root "$proj_dir" --name IdemCustom --id idem_custom_main > "$proj_dir/idem_custom_run1.log" 2>&1; then
        echo "  [FAIL IDEM] add_custom first run failed (see $proj_dir/idem_custom_run1.log)"
        FAILED=1
        return
    fi
    "$FRAMEWORK_DIR/add_custom_module.py" --project-root "$proj_dir" --name IdemCustom --id idem_custom_main > "$proj_dir/idem_custom_run2.log" 2>&1
    local rc_custom=$?
    ...
    if ! "$FRAMEWORK_DIR/add_uart_hw_module.py" --project-root "$proj_dir" --name IdemUartHw --id idem_uart_hw_main > "$proj_dir/idem_uart_hw_run1.log" 2>&1; then
        echo "  [FAIL IDEM] add_uart_hw first run failed (see $proj_dir/idem_uart_hw_run1.log)"
        FAILED=1
        return
    fi
    "$FRAMEWORK_DIR/add_uart_hw_module.py" --project-root "$proj_dir" --name IdemUartHw --id idem_uart_hw_main > "$proj_dir/idem_uart_hw_run2.log" 2>&1
    local rc_uart_hw=$?
    ...
    if ! $PYTHON_CMD "$FRAMEWORK_DIR/create_project.py" --validate-manifest "$proj_dir/hulotte.project.json" > "$proj_dir/idem_manifest.log" 2>&1; then
