...
IdemCustom::IdemCustom(const int n_elmts)
: Stateful(), n_elmts(n_elmts)
{
    const std::string name = "IdemCustom";
    this->set_name(name);
    ...
    {
        auto &mod = static_cast<IdemCustom&>(m);
...
IdemCustom* IdemCustom::clone() const
{
    auto m = new IdemCustom(*this);
    m->deep_copy(*this);
    return m;
}

void IdemCustom::_process(const int* in, int* out, const int frame_id)
{
    // Minimal example: copy input to output and print trace
    // std::cout << "IdemCustom processing frame " << frame_id << std::endl;
